...
ggplot(fig_data, aes(years, price))+
  geom_line()

fig_data$stock_id= rep("stuck_1",20)

stock_1_time_series<- fig_data
####Example

stock_id<- rep("stuck_2",20)

years<- seq(from=2001, to=2020, by=1)

price<- rnorm(20,mean = 10,sd = 3)

stock_2_time_series<- tibble(stock_id, "year"=years, "stock_price"=price)

all_stock_time_series<- rbind(stock_1_time_series,stock_2_time_series)

ggplot(all_stock_time_series, 
       aes(year, stock_price, group=stock_id, linetype=stock_id, color= stock_id))+
  geom_line()+
  facet_wrap(~stock_id)
...
ggplot(fig_data, aes(year, Ideology, group= party, color= party, linetype= party))+
  geom_line()+
  scale_color_manual(values = c("blue3", "red3"))+
  scale_linetype_manual(values = c(8,8,8, 8))
  
#########(Learning New Figures)
